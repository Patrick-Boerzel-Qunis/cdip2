...
from vst_data_analytics.preparation import read_data, cast_types, get_column_types, get_column_mapping
...
df_bisnode = read_data(spark, f"abfss://landing@vtl0cdip0dev0std.dfs.core.windows.net/cdip_test/data/01_bisnode_2023_7_V.{version}_0.parquet", COLUMN_DEFINITIONS["Bisnode"])
df_bisnode
...
df_bisnode_primus = read_data(spark, f"abfss://landing@vtl0cdip0dev0std.dfs.core.windows.net/cdip_test/data/02_bisnode_primus_2023_7_V.{version}_5.parquet", COLUMN_DEFINITIONS["BisnodePrimus"])
df_bisnode_primus
...
col_types = {**get_column_types(COLUMN_DEFINITIONS["Bisnode"]), **get_column_types(COLUMN_DEFINITIONS["BisnodePrimus"])}
col_mappings = {**get_column_mapping(COLUMN_DEFINITIONS["Bisnode"]), **get_column_mapping(COLUMN_DEFINITIONS["BisnodePrimus"])}
col_types = {col_mappings[col_name]: col_type for col_name, col_type in col_types.items()}
cast_types(spark.createDataFrame(df), col_types).write.mode("overwrite").option("overwriteSchema", "True").saveAsTable("`vtl-dev`.landing.t_dnb")
